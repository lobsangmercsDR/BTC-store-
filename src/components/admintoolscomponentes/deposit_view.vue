<template>
    <div>
      ...
      <table class="min-w-full bg-white shadow-md rounded-lg overflow-hidden">
        ...
        <tbody>
          <tr v-for="transaction in filteredTransactions" :key="transaction.id" class="text-gray-700">
            ...
            <td class="py-3 px-4 border-b border-gray-300">{{ transaction.wallet }}</td>
      ...
      <div class="flex justify-between mt-4">
        <button @click="previousPage" :disabled="currentPage === 1" class="px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-md transition-colors duration-300 ease-in-out" :class="{'cursor-not-allowed': currentPage === 1}">
          Previous
        </button>
        <div class="text-gray-700">
          Page {{ currentPage }} of {{ totalPages }}
        </div>
        <button @click="nextPage" :disabled="currentPage === totalPages" class="px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-md transition-colors duration-300 ease-in-out" :class="{'cursor-not-allowed': currentPage === totalPages}">
          Next
        </button>
      </div>
    </div>
